...
export default function Cadastro() {
  ...
  const handleCadastro = async () => {
    ...
    if (email.trim() !== emailconfirmation.trim()) {
      Alert.alert(
        "Atenção ⚠️",
        "Os e-mails não coincidem. Verifique a digitação.",
      );
      return;
    }
    if (password.trim() === "") {
    ...
    try {
      ...
      if (resposta.ok) {
        ...
        // expose actionable message to developer/user
        const hint = erroDoServidor.includes(
          "API access from non-browser environments",
        )
          ? 'Habilite "API access from non-browser environments" no painel do EmailJS (https://dashboard.emailjs.com/admin/account/security)'
          : "Verifique as credenciais do EmailJS.";
        Alert.alert("Falha ao enviar o e-mail", hint);
      }
    } catch (error) {
        ...
        {etapa === 1 ? (
          <>
            <Text style={styles.title}>
              Cadastre-se para poder assinar um plano
            </Text>
